...
2. **Rules-Based Intake Qualifier** (`rules-based-intake-qualifier.json`) — Scores every new inquiry against your firm's own practice areas and urgency signals, replies appropriately based on fit, and immediately alerts an attorney for the highest-fit leads — so attorney time goes to real cases, not screening calls.
