...
This template touches more genuinely new integration surface than most others in this catalog (IMAP email polling, Google Drive folder search/create). Budget extra time for the first test pass; this is not a "paste and go" template the way some others are.
...
- A Google Drive account (reuse the one from PAC-57 if already deployed — no separate Drive folder needed for this template, see Step 3)
...
## Step 3 — Set n8n Variables (2 min)

| Variable | What to enter |
|---|---|
| `CLIO_BASE_URL` | Same value as your other Clio-integrated templates, e.g. `https://app.clio.com` or `https://eu.app.clio.com` |
| `GENERATED_DOCS_FOLDER_ID` | A Drive folder ID matter subfolders are created under. If PAC-57 is already deployed, reuse its `GENERATED_DOCS_FOLDER_ID` value — one folder covers both templates. Otherwise create a folder and copy its ID from the URL: `https://drive.google.com/drive/folders/THIS_PART_IS_THE_ID` |
| `FIRM_FROM_EMAIL` | Reuse the value already set for other templates — used both as the alert email's From address and as the recipient for filing-exception alerts |
...
No new variables to create if PAC-57 is already deployed — this template reuses `GENERATED_DOCS_FOLDER_ID` and `FIRM_FROM_EMAIL` rather than introducing its own.

The workflow creates one subfolder per matter under `GENERATED_DOCS_FOLDER_ID` automatically the first time a document arrives for it — you don't need to pre-create matter folders.
...
- A new folder appears under `GENERATED_DOCS_FOLDER_ID`, named after the matter's display number.
- The attachment appears inside it, renamed with today's date prefixed.
- No alert email is sent.

**Test 2 — the exception path.** Email the intake mailbox from an address that is *not* a Clio contact. Confirm `FIRM_FROM_EMAIL` receives an alert with the original attachment and a clear reason ("No Clio contact matches this sender's email address").
...
| Matter's Drive folder doesn't exist yet | Created automatically under `GENERATED_DOCS_FOLDER_ID`, then filed |
...
| Deployability test | Reuses Clio, Google Drive, and Drive folder from PAC-57 if already deployed elsewhere in this catalog; new setup is just one IMAP mailbox |
